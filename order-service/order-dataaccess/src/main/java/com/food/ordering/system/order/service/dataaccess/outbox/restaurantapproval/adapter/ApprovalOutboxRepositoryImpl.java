    ...
    public Optional<List<OrderApprovalOutboxMessage>> findByTypeAndOutboxStatusAndSagaStatus(String sagaType,
        ...
        return Optional.of(approvalOutboxJpaRepository.findByTypeAndOutboxStatusAndSagaStatusIn(sagaType,
                ...
                .orElseThrow(() -> new ApprovalOutboxNotFoundException(
                        String.format("Approval outbox object could not be found for saga type %s", sagaType)))
                .stream()
                .map(orderApprovalEntity -> approvalOutboxDataAccessMapper
                        .approvalOutboxEntityToOrderApprovalOutboxMessage(orderApprovalEntity)).collect(Collectors.toList()));
    }
